...
from peft_pretraining.dataloader import PreprocessedIterableDataset, MyDataset
...
def parse_args(args):
    ...
    parser.add_argument("--use_hf_model", default=True, action="store_true")
...
def evaluate_model(model,   global_rank, world_size, device, batch_size):
    ...
    val_dataset = MyDataset(val_data,batch_size=args.batch_size, max_length=args.max_length)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=None, num_workers=args.workers)
    ...
    for batch in val_dataloader: 
        ...
        evaluated_on_tokens += (batch['input_ids'].shape[0] * batch['input_ids'].shape[1])
...
def main(args):
    ...
    if global_rank == 0:
        wandb.init(project="el-llama-3bn")
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        tokens_seen += (batch['input_ids'].shape[0] * batch['input_ids'].shape[1]) * world_size
        ...
        if update_step % args.eval_every == 0:
            ...
            total_loss, evaluated_on_tokens = evaluate_model(
                model,   global_rank, world_size, device, args.batch_size
            )
            if global_rank == 0:
                wandb.log({
